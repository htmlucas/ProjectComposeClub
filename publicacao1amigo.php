...
try {
   ...
   $stmt->execute();
	...
	$stmt = $conn->prepare("SELECT * from comentarios,usuario where usuario.id = comentarios.usuario_id and publicacao_id = $idi;");
   $stmt->execute();
...
<div class="navbar navbar-inverse" id="teste">
    <img src="img/log1.png" alt="nada.png" width="100px" height="100px">
    <form>
...
<div class="fb-profile-block">
    <div class="fb-profile-block-thumb">
        <img id="mina" src="img_usuario/<?php echo $publi['fotocapa'];?>" alt="" title="">
    </div>
    <div class="profile-img">
        <a href="teste.php"><img src="img_usuario/<?php echo $publi['fotoperfil'];?>" alt="" title=""></a>
      ...
      <div class="container">
          <div class="row">
              <div class="col-md-6 col-sm-6">
                    <div class="texto">
                           <a href="alterarcomposicao.php?id=<?php echo $publi['idpost'];?>"><button  class="edit">Editar Composição</button></a>
                           <a href="apagarcomposicao.php?id=<?php echo $publi['idpost'];?>"><button class="edit">Excluir Composição</button></a>
                           <div class="composicao">
                            <p><?php echo $publi['texto'];?></p>
                            </div>
                            <div class="comentario">
                                <div class="coment">
                                    <img class="pull-left" src="img_usuario/<?php echo $publi['fotoperfil'];?>" style="height:24px;width:24px;border-radius:100px ;margin:10px 5px 10px 10px;" alt="">
                                ...
                                <div class="mostrarcomente">
                                    <?php foreach ($publii as $row){?>
                                        <div class="mostrarcoment"><img class="pull-left"src="img_usuario/<?php echo $row['fotoperfil'];?>" style="height:24px;width:24px;border-radius:100px ;margin:0px 15px 20px 10px;" alt=""><p><?php echo $row['nomedeusuario'];?></p><p class="text-right"><?php echo $row['data_hora'];?></p><div class="comentario"><p><?php echo $row['comentario'];?></p></div></div>
                                    <?php }?> 
                                </div>
...
<script>    
					...
					$.ajax({
					  beforeSend: function() { //Antes de enviar o AJAX
						$(".mostrarcoment").prepend('<img id="loading" style="width: 60px;" src="img/loading.gif">');
					  },
					  ...
					  success: function(result) { //Resultado retornado do PHP 'result'
						if(result.deucerto){
							...
							$("#loading").remove();
							$(".mostrarcomente").prepend('<div class="mostrarcoment"><img class="pull-left"src="img_usuario/<?php echo $publi['fotoperfil'];?>" style="height:24px;width:24px;border-radius:100px ;margin:0px 15px 20px 10px;" alt=""><p><?php echo $publi['nomedeusuario'];?></p><p class="text-right">'+data+'</p><div class="comentario"><p>'+$("#textarea").val()+'</p></div></div>');        
						} else {
                ...
                $.ajax({
                    type: "GET",
                    url: "curtidas.php?acao=curtir",
                    dataType: "html",
                    success:function(response){
                        $('#contalikes').html(response);
                        $('#button').hide();
                        $('#buttondislike').show();
                }
             ...
             $(".buttondislike").click(function(){
                $.ajax({
                    type: "GET",
                    url: "curtidas.php?acao=descurtir",
                    dataType: "html",
                    success:function(response){
                        $('#contalikes').html(response);
                        $('#button').show();
	...
	$.ajax({
		...
		success: function(resultado){
            if(resultado['deucerto'] = true){
                $('#btnlike fa fa-thumbs-up').attr('onclick','dislike('+idpost+')');
                $('#btnlike fa fa-thumbs-up').removeClass('btnlike fa fa-thumbs-up');
               $('#btnlike fa fa-thumbs-up').addClass('btnlike');
            }
